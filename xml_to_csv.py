...
import updated_street_name
import update_zipcode

OSM_PATH = "sample_houston.osm"
...
def load_new_tag(element, secondary, default_tag_type):
    ...
    if secondary.attrib['k'].startswith('addr:') == 1 and secondary.attrib['k'].count(':') < 2:
        ...
        if field == 'street':
            value = updated_street_name.update_name(secondary.attrib['v'], mapping)
        elif field == 'postcode':
            value = update_zipcode.update_zipcode(secondary.attrib['v'])
